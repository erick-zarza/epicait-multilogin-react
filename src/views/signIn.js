...
import EmailLoginForm from "../components/signIn/emailSignIn";
const { Content } = Layout;
const EmailForm = Form.create()(EmailLoginForm);

class SignIn extends Component {
  constructor(props) {
    super(props);
    this.state = {
      redirectToProfileCard: false,
      loginIcon: (
        <div
          style={{
            width: 38,
    ...
    this.redirectToProfileCard = this.redirectToProfileCard.bind(this);
  }
  componentWillMount() {
    document.title = "Sign In";
  }
  redirectToProfileCard() {
    this.setState({
      redirectToProfileCard: true
    });
  }
  render() {
    const { redirectToProfileCard } = this.state;
    const { from } = "/profilecard";
    if (redirectToProfileCard) {
      return <Redirect to={"/profilecard/"} />;
    }
    return (
